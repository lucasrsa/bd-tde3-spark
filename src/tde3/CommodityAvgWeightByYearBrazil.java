...
// Questão 5
public class CommodityAvgWeightByYearBrazil {

    public static void main (String args[]){
        // Sets ERROR-only logging
        Logger.getLogger("org").setLevel(Level.ERROR);
        // habilita o uso de n threads
        SparkConf conf = new SparkConf().setAppName("commodityAvgWeightByYearBrazil").setMaster("local[*]");
        // cria o contexto da aplicacao
        JavaSparkContext sc = new JavaSparkContext(conf);

        // Carrega arquivo
        JavaRDD<String> rdd = sc.textFile("in/transactions.csv")
                // Filtra apenas linhas do Brasil
                .filter(l -> (l.split(";")[0].equals("Brazil") && (!l.split(";")[6].isEmpty())));
        ...
        // Escreve em arquivo
        pairRDD.coalesce(1).saveAsTextFile("output/CommodityAvgWeightByYearBrazil");
    }
